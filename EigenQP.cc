...
double solve_quadprog(Eigen::MatrixXd& G, Eigen::VectorXd& g0, const Eigen::MatrixXd& CE, const Eigen::VectorXd& ce0,
  ...
  std::ostringstream msg;
  {
    ...
    if (G.cols() >= mx || G.rows() >= mx || CE.rows() >= mx || CE.cols() >= mx || CI.rows() >= mx || CI.cols() >= mx ||
        ci0.size() >= mx || ce0.size() >= mx || g0.size() >= mx) {
      msg << "The dimensions of one of the input matrices or ublas::vectors were "
          << "too large." << std::endl
          << "The maximum allowable size for inputs to solve_quadprog is:" << mx << std::endl;
      throw std::logic_error(msg.str());
    }
  }
  int n = G.cols(), p = CE.cols(), m = CI.cols();
  if (static_cast<int>(G.rows()) != n) {
    msg << "The ublas::matrix G is not a square ublas::matrix (" << G.rows() << " x " << G.cols() << ")";
    throw std::logic_error(msg.str());
  }
  if (static_cast<int>(CE.rows()) != n) {
    msg << "The ublas::matrix CE is incompatible (incorrect number of rows " << CE.rows() << " , expecting " << n
        << ")";
    throw std::logic_error(msg.str());
  }
  if (static_cast<int>(ce0.size()) != p) {
    msg << "The ublas::vector ce0 is incompatible (incorrect dimension " << ce0.size() << ", expecting " << p << ")";
    throw std::logic_error(msg.str());
  }
  if (static_cast<int>(CI.rows()) != n) {
    msg << "The ublas::matrix CI is incompatible (incorrect number of rows " << CI.rows() << " , expecting " << n
        << ")";
    throw std::logic_error(msg.str());
  }
  if (static_cast<int>(ci0.size()) != m) {
    msg << "The ublas::vector ci0 is incompatible (incorrect dimension " << ci0.size() << ", expecting " << m << ")";
    throw std::logic_error(msg.str());
  }
  x.resize(n);
